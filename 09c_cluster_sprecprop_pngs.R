...
#---------------------------------------------------#
# Sorensen clusters
l <- paste0("outputs/", run_id, "/_tifs/individ_cluster_specprop_tif/", clust_names, ".tif")
habs <- rast(l)
...
for(i in 1:k){
  r <- habs[[i]]
  r <- r*100
  dest_fp <- paste0("outputs/", run_id, "/_pngs/individ_cluster_specprop_png/", clust_names[i], ".png")
  r[land == 1] <- NA
  r[r==0] <- NA
  ...
  temp_plot <- ggplot() +
    ...
    #                 palette = "berlin",
    #                 direction = 1) +
    scale_fill_distiller(name = "Percentage of species\nin cluster present",
                         na.value = "transparent",
                         palette = "YlOrRd",
                         direction = 1) +
